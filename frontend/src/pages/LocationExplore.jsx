...
export default function LocationExplore({ currentUser }) {
  ...
  const fetchLocationPosts = async () => {
    ...
    try {
      setError("");
      const res = await fetch(`${API_BASE}/api/posts/explore?search=${encodeURIComponent(decodedPlace)}`, {
        headers: authHeaders(),
      });
      if (res.ok) {
        const data = await res.json();
        // Filter specifically to location matches
        const filtered = data.filter(
          (post) => post.location && post.location.toLowerCase().includes(decodedPlace.toLowerCase())
        );
        setPosts(filtered);
      } else {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-brutal-green/20 py-12 px-4 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-brutal-green/20 py-12 px-4">
        ...
        {posts.length === 0 ? (
          <div className="neo-card bg-white border-4 border-black p-8 text-center shadow-[8px_8px_0px_0px_#000] my-12">
            ...
            <p className="font-bold text-gray-600 mb-6">
              Be the first to post from {decodedPlace}!
            </p>
            <Link
              to="/create"
